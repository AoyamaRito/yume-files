...
- For long novel txt ingest, do not convert the whole source directly into a polished world file. Follow `novelSourceIngest.workflow.yume.js`:
  1. Create source index, term index, an empty relation index, a settings catalog, multiple empty settings collections, and redundant fact staging first (via `tools/novel-source-ingest.js`).
  2. Extract facts and relations from chunks (via `tools/novel-fact-extract.js` and `tools/novel-relation-extract.js`).
  3. Normalize focused entries into setting collections (via `tools/novel-settings-normalize.js`).
