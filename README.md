...
relations-index、world-facts の中間 artifact を作ってください。関連は空の graph から
証拠付きで徐々に記述し、重複や矛盾は初期段階では残してください。
...
これは `.yume-work/` に `chunks/`、`terms.raw.jsonl`、`occurrences.jsonl`、`manifest.json` などの中間ファイルを作り、`novel.source.index.yume.js`、`novel.terms.index.yume.js`、`novel.relations.index.yume.js`、`novel.world.facts.yume.js` を生成します。中間ファイルを成功時に消す場合は `--clean-workdir` を付けます。
...
├── tools/
│   └── novel-source-ingest.js      # txt → source/term/relation/fact staging generator
├── examples/
...
This creates intermediate files under `.yume-work/` and emits source, term, empty relation, and empty world-fact staging `.yume.js` files. Use `--clean-workdir` when the intermediate files should be deleted after a successful run.
